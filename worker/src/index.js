  ...
  if (!hasWestBengal && uniqueCandidates.length > 0) {
    ...
    if (wbCandidates.length > 0) {
      ...
      if (replacementIndex >= 0) {
        finalNews[replacementIndex] = wbCandidates[0];
      }
    }
  }
...
Your job is to select the 8 most important and useful NEW Indian news stories from the provided candidates: EXACTLY 4 Bengali-language source stories and EXACTLY 4 English-language source stories.
...
- Return EXACTLY 4 objects whose candidate source language is "bn" and EXACTLY 4 objects whose candidate source language is "en".
- Preserve the source language assignment; never substitute one language for the other.
...
- Return EXACTLY 8 objects (no less, no more).
...
function validateGeminiResults(parsed, candidates) {
  ...
  for (const item of parsed) {
    ...
    if (original.language === "bn") {
      if (bnCount >= 4) continue;
      bnCount++;
    } else if (original.language === "en") {
      if (enCount >= 4) continue;
      enCount++;
    } else {
      continue;
    }
